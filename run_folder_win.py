...
class StableHair:
    ...
    def Hair_Transfer(self, source_image, reference_image, random_seed, step, guidance_scale, scale, controlnet_conditioning_scale, size=512):
        ...
        # load imgs
        source_image = self.source_image #Image.open(source_image).convert("RGB").resize((size, size))
        id = np.array(source_image)
        reference_image = np.array(Image.open(reference_image).convert("RGB").resize((size, size)))
        source_image_bald = np.array(self.source_image_bald) #  np.array(self.get_bald(source_image, scale=0.9))
    ...
    def get_bald(self, id_image, scale):
        ...
        self.source_image_bald = image
        return image


def _get_images_filenames_in_folder(folder_path):
    files = os.listdir(folder_path)
    jpg_files = [os.path.join(folder_path,f) for f in files if (f.endswith('.png') or f.endswith('.jpeg') or f.endswith('jpg')) and os.path.isfile(os.path.join(folder_path, f))]
    return jpg_files


def run_all():
    folder_path = 'D:/Projects/Stable-Hair/test_imgs/'
    output_path = 'D:/Projects/Stable-Hair/output'

    model = StableHair(config="D:/Projects/Stable-Hair/configs/hair_transfer_win.yaml", weight_dtype=torch.float32)
    ...
    for id_image in tqdm(source_files):
        # generate bald
        print("Generating bald image for ID image: ", id_image)
        model.source_image = Image.open(id_image).convert("RGB").resize((image_size, image_size))
        model.get_bald(model.source_image, scale=0.9)
        for ref_hair in ref_files:
            ...
            id, image, source_image_bald, reference_image = model.Hair_Transfer(id_image, ref_hair, random_seed = -1, step = 40, guidance_scale = 1.5, scale = 1, controlnet_conditioning_scale = 1, size = image_size)
